...
Before applying metrics or transformations, it is useful to inspect the structure of the dataset. `detect_variable_types` classifies columns as numerical, categorical, binary, identifiers, or high-cardinality categorical variables. `missing_value_report` shows where missing data appears. `validate_binary_target` checks that the target is suitable for AUC. `impute_missing_values` creates a cleaned copy by filling missing numerical values with the median and categorical values with the mode when `strategy = "auto"`.
...
```{r}
summary <- dataset_summary(analysis_data)
summary[c("n_rows", "n_columns", "numerical_columns", "categorical_columns")]
```
